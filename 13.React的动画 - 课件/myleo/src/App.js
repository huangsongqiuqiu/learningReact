...
class App extends Component {
  ...
  css(){
    return `
      ...
      .myTab ul{width:100%;border:1px solid black;top:40px;left:-1px;position:absolute;border-bottom:none;overflow:hidden;}
  ...
  render() {
    ...
    this.state.myArr.forEach((v,i)=>{
      aLi.push(<li onClick={this.myClick.bind(this,i)} key={i}>{v}</li>)
    })
    return (
      <div className="App">
        <style dangerouslySetInnerHTML={{__html:this.css()}}/>
        <Motion style={{myLeo:spring(this.state.v?this.state.myArr.length*40+this.state.myArr.length:0)}}>
            {({myLeo})=>
